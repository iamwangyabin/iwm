...
class DropPath(nn.Module):
    """对残差分支执行随机深度（Stochastic Depth）。

    使用位置: `Block` 中作为残差分支的正则化。
...
class MLP(nn.Module):
    """Transformer 前馈网络（两层全连接 + 激活 + dropout）。

    使用位置: `Block` 与 `CrossAttentionBlock` 的 MLP 子层。
    """
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_sdpa:
            sdpa_ctx = getattr(torch.nn.attention, "sdpa_kernel", None)
            if sdpa_ctx is None:
...
class Block(nn.Module):
    """ViT 基础块：LN + MHA + MLP 的残差结构。

    使用位置: `VisionTransformer` 与 `VisionTransformerPredictor` 的主干层。
    """
...
class PatchEmbed(nn.Module):
    ...
    使用位置: `VisionTransformer` 在 2D 图像时的 patch embedding。
...
class VisionTransformerPredictor(nn.Module):
    ...
    使用位置: `models/__init__.py:init_jepa_model` 通过 `vit_predictor*` 构造，
    作为 JEPA/IWM 预测器。
    """
    def __init__(
        self,
