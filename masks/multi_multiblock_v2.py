...
class MaskCollator(object):
    """多块掩码采样 collator（每次样本独立采样尺寸与位置）。

    用途: 为每个样本独立采样预测/编码块，增加多样性。
    使用位置: `masks.build_mask_collator(mask_type=multi_multiblock_v2)`，
    在 `train_jepa.py` 的 DataLoader 里作为 `collate_fn`。
    """
