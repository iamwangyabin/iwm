...
class MaskCollator(object):
    """多块掩码采样的 collator（按列表返回多组掩码）。

    用途: 返回 list 风格的掩码，配合 `JEPA(list_mask=True)` 处理多组 mask。
    使用位置: `masks.build_mask_collator(mask_type=multi_multiblock_list)`，
    与 `models/__init__.py` 中 `list_mask` 逻辑配合。
    """
